...
if( isset( $_POST['submitted'] ) ) {
	...
	if( trim( $_POST['checking'] ) !== '' ) {
		...
		if( ! isset( $hasError ) ) {
			...
			if( $sendCopy == true ) {
				$subject = __( 'Uw mail naar ', 'woothemes' ) . get_bloginfo( 'title' );
		...
		if(!hasError) {
			...
			jQuery.post(jQuery(this).attr( 'action'),formInput, function(data){
				jQuery( 'form#contactForm').slideUp( "fast", function() {
					jQuery(this).before('<p class="tick"><strong>Bedankt!</strong> Uw email is ontvangen.<br />Op werkdagen krijgt u binnen een dag een reactie op uw aanvraag.</p>');
				});
			});
		}
