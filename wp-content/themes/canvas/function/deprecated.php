        ...
        if ( $type_selector ) {
            ...
            if( isset( $woo_metabox['type'] ) && ( in_array( $woo_metabox['type'], woothemes_metabox_fieldtypes() ) ) ) {
                ...
                elseif ( $woo_metabox['type'] == 'calendar' ) {
                    ...
                    $output .= "\t\t" . '<input type="hidden" name="datepicker-image" value="' . get_template_directory_uri() . '/function/images/calendar.gif" />';
...
function woothemes_uploader_custom_fields( $pID, $id, $std, $desc ) {
    ...
    if ( $href )
        $uploader .= '<span class="woo_metabox_desc">' . $desc . '</span></td>' . "\n" . '<td class="woo_metabox_image"><a href="' . $upload . '"><img src="' . get_template_directory_uri() . '/function/thumb.php?src=' . $href . '&w=150&h=80&zc=1" alt="" /></a>';
...
function woo_custom_enqueue ( $hook ) {
    ...
    wp_register_script( 'jquery-ui-datepicker', get_template_directory_uri() . '/function/js/ui.datepicker.js', array( 'jquery-ui-core' ) );
    wp_register_script( 'jquery-input-mask', get_template_directory_uri() . '/function/js/jquery.maskedinput.js', array( 'jquery' ), '1.3' );
    wp_register_script( 'woo-custom-fields', get_template_directory_uri() . '/function/js/woo-custom-fields.js', array( 'jquery', 'jquery-ui-tabs' ) );
...
function woo_custom_enqueue_css () {
    ...
    global $pagenow;
    wp_register_style( 'woo-custom-fields', get_template_directory_uri() . '/function/css/woo-custom-fields.css' );
    wp_register_style( 'jquery-ui-datepicker', get_template_directory_uri() . '/function/css/jquery-ui-datepicker.css' );
...
if ( ! function_exists( 'woothemes_machine' ) ) {
    ...
    function timthumb_deprecated( $args = array() ) {
        ...
        foreach( $src_arr as $key => $custom_field ) {
            ...
            $img_url = esc_url( get_template_directory_uri() . '/function/thumb.php?src=' . $custom_field . '&amp;w=' . $width . '&amp;h=' . $height . '&amp;zc=1&amp;q=' . $quality . $alignment );
