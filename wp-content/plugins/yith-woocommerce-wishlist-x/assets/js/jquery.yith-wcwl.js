jQuery( document ).ready( function( $ ){
	...
	function call_ajax_add_to_wishlist( el ) {
		...
		$.ajax({
			...
			success: function( response ) {
				...
				$( '#yith-wcwl-title' ).html( response_message );
				if( response_result == "true" ) {
					...
					$( '#yith-wcwl-message' ).html(
						...
						'<p>' + response.product['title'] + '</p>' +
						response.product['price']
					);
					add_go_to_wishlist_button(response.wishlist_url);
					add_close_button('verder winkelen');
	...
	function add_wishlist_popup() {
		if( $( '#yith-wcwl-popup-message' ).length == 0 ) {
			var title = $( '<div>' )
					.attr( 'id', 'yith-wcwl-title' ),
				message = $( '<div>' )
					.attr( 'id', 'yith-wcwl-message' ),
				buttons = $( '<div>' )
					.attr( 'id', 'yith-wcwl-buttons' ),
				cross = $( '<div>' )
					.attr( 'id', 'yith-wcwl-cross' )
					.addClass( 'dashicons dashicons-dismiss' )
					.attr( 'onclick', 'close_yith_wcwl_popup_message();'),
				wrapper = $( '<div>' )
					.attr( 'id', 'yith-wcwl-popup-wrapper')
					.append( title )
					.append( message )
					.append( buttons )
					.append( cross ),
	...
	function add_go_to_wishlist_button(url) {
		var button = $( '<a>' )
			.attr( 'href', url)
			.text('naar verlanglijst');
		$('#yith-wcwl-buttons').append( button );
	}

	function add_close_button(text) {
		var button = $( '<a>' )
...
function close_yith_wcwl_popup_message() {
	jQuery('#yith-wcwl-title').html('');
	jQuery('#yith-wcwl-message').html('');
	jQuery('#yith-wcwl-buttons').html('');
	jQuery( '#yith-wcwl-popup-message' ).hide();
}

function showSendEmailPopup() {
	jQuery('#yith-wcwl-title').html('VERLANGLIJST DOORSTUREN');

	jQuery('<div>').append(
		jQuery('<div>').append(
			jQuery('<span>').text( 'E-mail ontvanger:' )
		)
	).append(
		jQuery('<div>').append(
			jQuery('<input>').attr( { type: 'email', id: 'to-email' } )
		)
	).appendTo('#yith-wcwl-message');
	jQuery('<div>').append(
		jQuery('<div>').append(
			jQuery('<span>').text( 'Voeg een persoonlijk bericht toe:' )
		)
	).append(
		jQuery('<div>').append(
			jQuery('<input>').attr( { type: 'text', id: 'to-message' } )
		)
	).appendTo('#yith-wcwl-message');
	jQuery('<div>').append(
		jQuery('<div>').append(
			jQuery('<span>').text( 'Uw e-mailadres:' )
		)
	).append(
		jQuery('<div>').append(
			jQuery('<input>').attr( { type: 'email', id: 'from-email' } )
		)
	).appendTo('#yith-wcwl-message');
	jQuery('<div>').append(
		jQuery('<div>').append(
			jQuery('<span>').text( 'Uw naam:' )
		)
	).append(
		jQuery('<div>').append(
			jQuery('<input>').attr( { type: 'text', id: 'from-name' } )
		)
	).appendTo('#yith-wcwl-message');

	add_button('annuleer', 'close_yith_wcwl_popup_message();');
	add_button('verstuur', 'yith_wcwl_send_wishlist_by_email();');

	jQuery( '#yith-wcwl-popup-message' ).show();
}

function add_button(text, onclick, href) {
	var button = jQuery( '<a>' )
		.attr( 'href', href || 'javascript:void(0)' )
		.attr( 'onclick', onclick || '' )
		.text(text || 'button_text');
	jQuery('#yith-wcwl-buttons').append( button );
}

function yith_wcwl_send_wishlist_by_email() {
	var table = jQuery('table.cart.wishlist_table'),
		wishlist_token = table.data( 'token'),
		wishlist_id = table.data( 'id' ),
		data = {
			'action': 'send_wishlist_by_email',
			'wishlist-token': wishlist_token,
			'wishlist-id': wishlist_id,
            'wpnonce': jQuery('#yith_wcwl_form_nonce').val(),
            'to-email': jQuery('#to-email').val(),
			'to-message': jQuery('#to-message').val(),
			'from-email': jQuery('#from-email').val(),
			'from-name': jQuery('#from-name').val()
		};

	jQuery.post( yith_wcwl_l10n.ajax_url, data, function(response){
		console.log(response);
		alert(response.data);
		if(response.success) {
			close_yith_wcwl_popup_message();
		}
	} );
}
