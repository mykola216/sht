...
Text Domain: woocommerce-watermark
	...
	Copyright: © 2009-2011 Sixty One Designs.
 ...
 **/
if (is_admin()) {
	$woo_plugin_updater_watermark = new WooThemes_Plugin_Updater( __FILE__ );
	$woo_plugin_updater_watermark->api_key = 'bb91bd9009efe9e629058e32c4e1074c';
	$woo_plugin_updater_watermark->init();
}
		...
		if ( $watermark_image ) {
			...
			if ( is_file( $watermark_image_path ) ) {
				...
				if ( $original_image && $overlay ) {
					...
					if ( _WATERMARK_DEBUG ) mail( _WATERMARK_DEBUG_EMAIL, 'watermark - SUCCESS', $filepath.var_export( $_REQUEST, true ) );
					return true;
