...
if (isset ( $_POST ['cmd'] ) && $_POST ['cmd'] == 'batchUpdate') {
	...
	if ($updated_rows_cnt >= 1)
		$encoded ['msg'] = "<b>" . $updated_rows_cnt . "</b> " . __('Records Updated Successfully',$sm_text_domain);
	else
		$encoded ['msg'] = __("No Records Updated",$sm_text_domain);
        // ob_clean();
...
if (! function_exists( 'get_order_status_string' )) {
	function get_order_status_string ( $status_code ) {
		switch ( intval ( $status_code ) ) {
			...
			case 1:
				return __('Incomplete Sale',$sm_text_domain);
				break;
				
			case 2:
				return __('Order Received',$sm_text_domain);
				break;
				
			case 3:
				return __('Accepted Payment',$sm_text_domain);
				break;
				
			case 4:
				return __('Job Dispatched',$sm_text_domain);
				break;
				
			case 5:
				return __('Closed Order',$sm_text_domain);
				break;
				
			default:
				return __('Payment Declined',$sm_text_domain);
				break;
