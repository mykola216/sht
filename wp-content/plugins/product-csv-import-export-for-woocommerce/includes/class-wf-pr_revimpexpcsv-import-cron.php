...
class WF_PrRevImpExpCsv_ImportCron {
      ...
      if(!defined('WP_LOAD_IMPORTERS'))
        ...
        if($this->handle_ftp_for_autoimport()){
            if($this->settings['rev_auto_import_profile']!== ''){
                ...
                $GLOBALS['WF_CSV_Product_Review_Import']->log->add( 'csv-import', __( 'Failed processing import. Reason:'.$this->error_message, 'wf_csv_import_export' ) );
            }
	...
	$GLOBALS['WF_CSV_Product_Review_Import']->import_end();
        ...
        die();
            ...
            $GLOBALS['WF_CSV_Product_Review_Import']->log->add( 'csv-import', __( 'Fetching file failed. Reason:'.$this->error_message, 'wf_csv_import_export' ) );
        }
