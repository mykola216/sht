...
class WC_Order_Export_Manage {
	...
	private static function apply_defaults( $mode, $settings ) {
		$defaults = array(
			'mode'                                           => $mode,
	...
	static function import_settings( $data ) {
		...
		if( isset( $data[ self::settings_name_now ] ) ) { // import ALL
		...
		elseif( isset( $data["mode"] ) ) { // OR import single ?
			$import_type = self::get_settings_name_for_mode( $data["mode"] );
			if( in_array( $import_type, $allowed_options ) ) {
				if( $import_type == self::settings_name_now ) {
